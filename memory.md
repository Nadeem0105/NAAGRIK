...
8. **Pre-deployment Stability and Security Audit Fixes**: 
    *   **AI Fallback Degradation**: Fixed an issue where a timeout in the Groq AI service resulted in a generic static response rather than gracefully degrading to the robust local keyword/rule-based classifier. 
    *   **Dashboard Spam Exclusion**: Fixed an issue in `dashboard_service.py` where issues flagged as spam by the AI pipeline were incorrectly inflating public impact statistics and department rankings. 
    *   **Hotspot Clustering Spam Exclusion**: Fixed the geospatial DBSCAN clustering query (`issue_repo.py`) to properly strip out spam-flagged issues, preventing bots from artificially creating hotspots on the map.
    *   **Foreign Key Integrity**: Added missing `cascade="all, delete-orphan"` SQLAlchemy constraints to `verifications`, `comments`, and `status_history` on the `Issue` model to prevent fatal PostgreSQL 500 errors during row deletion.
